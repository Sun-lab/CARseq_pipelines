...
replicates = 201:210  # with noise in cell fractions (rho)
metrics_list = list()
for (n in n_list) {
...
write.csv(metrics, file = "../results/compare_methods_metrics_with_noise_0.1.csv", quote = FALSE, row.names = FALSE)

replicates = 401:410  # with noise in cell fractions (rho)
metrics_list = list()
for (n in n_list) {
  for (fold_change in fold_changes) {
    DE_pattern_list = list(c(fold_change, 1, 1)
...
# create a data frame
metrics = do.call(rbind, metrics_list)
write.csv(metrics, file = "../results/compare_methods_metrics_size_factor_1.2.csv", quote = FALSE, row.names = FALSE)


replicates = 301:310  # with noise in cell fractions (rho)
metrics_list = list()
for (n in n_list) {
  for (fold_change in fold_changes) {
    DE_pattern_list = list(c(fold_change, 1, 1)
...
# create a data frame
metrics = do.call(rbind, metrics_list)
write.csv(metrics, file = "../results/compare_methods_metrics_size_factor_2.csv", quote = FALSE, row.names = FALSE)
...
############################################################
# generate plots (replicates 201..210, with noise of N(0, 0.1) on logit scale)
############################################################
library(tidyr)
library(ggplot2)
...
pdf("../figures/compare_methods_fdr_with_noise_draft_version_p1.pdf", height=4, width=7)
# 7in x 7in:
# ggarrange(
...
pdf("../figures/compare_methods_fdr_size_factor_1.2_draft_version_p1.pdf", height=4, width=7)
# 7in x 7in:
# ggarrange(
#   g1_with_covariates %+% subset(metrics_with_covariates, DE_pattern_index == 1),
#   g1_without_covariates %+% subset(metrics_without_covariates, DE_pattern_index == 1),
...
############################################################
# generate plots (replicates 301..310, size_factor 2)
############################################################
library(tidyr)
library(ggplot2)
...
pdf("../figures/compare_methods_fdr_size_factor_2_draft_version_p1.pdf", height=4, width=7)
# 7in x 7in:
# ggarrange(
#   g1_with_covariates %+% subset(metrics_with_covariates, DE_pattern_index == 1),
#   g1_without_covariates %+% subset(metrics_without_covariates, DE_pattern_index == 1),
