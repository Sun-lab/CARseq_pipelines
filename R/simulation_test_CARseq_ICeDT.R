...
################################################### 
test_CARseq_ICeDT = function(config, ncpus) {
  ...
  without_RIN_bench_parallel = peakRAM({
    without_RIN = run_CARseq(count_matrix = observed_read_count,
                             cellular_proportions = icedt_rho,
                             groups = x,
                             formula = NULL,
  ...
  # Caution: this is not accurate. When using parallel, peakRAM is not able to get all the memory usage:
  without_RIN$Peak_RAM_Used_MiB = without_RIN_bench_parallel$Peak_RAM_Used_MiB
  ...
  save(without_RIN, with_RIN, file=file.path("../simulation", config, "CARseq_ICeDT_res.RData"))
  
  pdf(file.path("../simulation", config, "CARseq_ICeDT_pvalue_distribution.pdf"), height=8, width=10)
  par(mfrow=c(2,3))
