...
configs = list.files(pattern="../data/^n_.*_DE_pattern_.*_1_1_replicate_.?[0123456789]$") # d_1_1 from replicate 1 to 99
...
# for (config in configs) {
  ...
  # TODO: generate a scatterplot of three cell types to put in Supp. Mat.
  rho = rho_with_noise
  ...
  if (!file.exists(file.path(config_noise, "TOAST_res_TPM.RData"))) {
  ...
  if (!file.exists(file.path(config_noise, "CARseq_res.RData"))) {
    test_CARseq(config_noise, ncpus = 12)
  }
  ...
  if (!file.exists(file.path(config_size_factor, "TOAST_res_TPM.RData"))) {
  ...
  if (!file.exists(file.path(config_size_factor, "CARseq_res.RData"))) {
    test_CARseq(config_size_factor, ncpus = 12)
  }
